...
const API_KEY = '6f0b11685c8dcd1df81d8f74c51ebc44';
...
const formatCurrent = (data) => {
    ...
    return {
        ...
        speed,
        details, icon: iconUrlFromCode(icon),
        formattedLocalTime,
        dt,
        timezone, lat,
        lon
...
const formatForecastWeatherData = (secs, offset, data) => {
    console.log(secs)
    // hourly
    const hourly = data
        .filter((f) => f.dt > secs)
...
const getFormattedWeatherData = async (searchParams) => {
    ...
    const formattedForecastWeather = await getWeatherData('forecast', { lat, lon, units: searchParams.units })
        .then((d) => formatForecastWeatherData(dt, timezone, d.list));
